...
describe('MatchListItem closed summary wiring (K1)', () => {
  ...
  it('structures the calendar with tabs and next / ensuite / finished panels', () => {
    ...
    assert.match(calendar, /classifyMatchPhase/)
    assert.match(calendar, /liveItems/)
    assert.match(calendar, /awaitingItems/)
    assert.match(calendar, /Match en cours/)
    assert.match(calendar, /Résultat en attente/)
    assert.match(calendar, /calendar-awaiting-heading/)
